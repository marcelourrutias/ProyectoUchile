...
def reg_float_to_bin(num_str):
	if num_str[0] == 'f' and num_str[1]!='a' and num_str[1]!='s' and num_str[1]!='t':
		return num_str_to_bin(num_str[1:], 10, 5, False)
	else:
...
def ex_utype(data):
	rd_bin = reg_to_bin(data.rd)
	imm_bin = val_to_bin(data.imm, 20, True)

	return imm_bin + rd_bin + data.opcode
...
handlers = {
	...
	"sll": 		CommandHandler(parse_reg_reg_reg, ex_rtype, CommandData("0110011", "001", "0000000")),
	"slt": 		CommandHandler(parse_reg_reg_reg, ex_rtype, CommandData("0110011", "010", "0000000")),
	"sltu": 	CommandHandler(parse_reg_reg_reg, ex_rtype, CommandData("0110011", "011", "0000000")),
	"xor": 		CommandHandler(parse_reg_reg_reg, ex_rtype, CommandData("0110011", "100", "0000000")),
	"srl": 		CommandHandler(parse_reg_reg_reg, ex_rtype, CommandData("0110011", "101", "0000000")),
	"sra": 		CommandHandler(parse_reg_reg_reg, ex_rtype, CommandData("0110011", "101", "0100000")),
	"or": 		CommandHandler(parse_reg_reg_reg, ex_rtype, CommandData("0110011", "110", "0000000")),
	"and": 		CommandHandler(parse_reg_reg_reg, ex_rtype, CommandData("0110011", "111", "0000000")),
	"slli": 	CommandHandler(parse_reg_reg_imm, ex_sitype, CommandData("0010011", "001", "0000000")),
	"srli": 	CommandHandler(parse_reg_reg_imm, ex_sitype, CommandData("0010011", "101", "0000000")),
	"srai": 	CommandHandler(parse_reg_reg_imm, ex_sitype, CommandData("0010011", "101", "0100000")),
	...
	"fnmadd.s":	CommandHandler(parse_reg_reg_reg_reg, ex_r4type, CommandData("1001111", "111")),
	"fnmsub.s":	CommandHandler(parse_reg_reg_reg_reg, ex_r4type, CommandData("1001011", "111")),
	...
	"feq.s":	CommandHandler(parse_reg_reg_reg, ex_rtype_int_float, CommandData("1010011", "010", "1010000")),
	"flt.s":	CommandHandler(parse_reg_reg_reg, ex_rtype_int_float, CommandData("1010011", "001", "1010000")),
	"fle.s":	CommandHandler(parse_reg_reg_reg, ex_rtype_int_float, CommandData("1010011", "000", "1010000")),
	...
	"fcvt.s.wu":CommandHandler(parse_reg_reg_1	, ex_rtype_float_int, CommandData("1010011", "111", "1101000")),
...
if __name__ == "__main__":
	...
	Texto= open(input("Ingrese nombre del archivo: "))
	...
	with open("text_in","w") as file:
		Texto= open("result")
		for line in Texto:
			lineVal=line.strip('\n')
			lineVal=lineVal.strip().lower()
			parts = lineVal.split(" ")
			title = parts[0]
			tail = "".join(parts[1:])
			handler = handlers[title]
			handler.parse(tail)
			output = hex(int(handler.execute(),2))
			output2=output.split('x')[1]
			while (len(output2)<8):
				output2='0'+output2
